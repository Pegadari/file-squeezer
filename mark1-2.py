...
def main() -> None:
    ...
    target0 = TARGET_10KB   # unsigned int of a file's binary
    targets = 10           # number of consecutive targets to test
    jump = 1234
    do_decode = False

    start = time.time()

    for i in range(0, targets * jump, jump):
        target = target0 + i
        target_bits = int_to_bits(target)



        # VECTOR
        frequency_vector = squeeze(target)


        theoretical_min = sum(list(map(int_to_bits, frequency_vector)))
        fixed_width_1 = 1 + len(frequency_vector) * int_to_bits(max(frequency_vector))
        fixed_width_2 = vector_size_2(frequency_vector)
        fixed_width_4 = vector_size_4(frequency_vector)
        elias_delta = len(elias_delta_encode(frequency_vector))
        elias_gamma = len(elias_gamma_encode(frequency_vector))
        elias_omega = len(elias_omega_encode(frequency_vector))



        print(f"""+{i}: {target_bits} bits
    fw1:   {target_bits / fixed_width_1}
    fw2:   {target_bits / fixed_width_2}
    fw4:   {target_bits / fixed_width_4}
    delta: {target_bits / elias_delta}
    gamma: {target_bits / elias_gamma}
    omega: {target_bits / elias_omega}
    min:   {target_bits / theoretical_min} 
""")        
    ...
    print(f"Average squeeze time: {duration / targets} s")
